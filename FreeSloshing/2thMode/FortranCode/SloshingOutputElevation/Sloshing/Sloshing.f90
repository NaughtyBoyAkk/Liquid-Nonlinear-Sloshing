    ...
    include 'mkl_blas.fi'
    !
    real(kind=8),parameter::t0 = 0.0D0, t1 = 12.0D0, timestep = 0.012D0
    !
    ...
    do while (ti < t1)
        write(*,'(A,F9.4)') " ti= ",ti
        !analytical_eta = -Amp * cos(waveNum*(Cx(EN1,1)+tankL)) * cos(omegaf*ti) ! 液面中点
        !analytical_eta = Amp * cos(waveNum*(Cx(0,1)+tankL / 2.0D0)) * cos(omegaf*ti) ! 右侧角点
